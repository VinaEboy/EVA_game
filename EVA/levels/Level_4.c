...
level_4 *level_4_info_create(game_state *state, int X_SCREEN, int Y_SCREEN, int checkpoint) {
    level_4 *level_4_info = malloc(sizeof(level_4));
    if (!level_4_info) return NULL;
    level_4_info->camera_x = 0;
    level_4_info->FLOOR = Y_SCREEN*0.07;

    level_4_info->checkpoint = state->checkpoint;
    if (level_4_info->checkpoint == 0)
        level_4_info->player = create_player(state->player_progress->dificulty, 0, 0, X_SCREEN, Y_SCREEN);
 

    level_4_info->num_platforms = 2;
    create_level_4_platforms(level_4_info, X_SCREEN, Y_SCREEN);

    level_4_info->exit_level_4 = 0;
...
void start_level_4(game_state *state, level_4 **level_4_info, entities_sprites *sprites, int X_SCREEN, int Y_SCREEN, int checkpoint) {
    ...
    *level_4_info = level_4_info_create(state, X_SCREEN, Y_SCREEN, checkpoint);

    sprites->level_4_background = al_load_bitmap("images/cenario/level_4_background.png");
    sprites->level_4_ground = al_load_bitmap("images/cenario/level_4_ground.png");
    ...
    if (// Cenário
        !sprites->level_4_background || !sprites->level_4_ground || 
        // Jogador
    ...
    state->level_4_started = 1;
}

void level_4_update(level_4 *level_4_info, game_state *state, int X_SCREEN, int Y_SCREEN) {
    Player *player = level_4_info->player;

    //verifica gravidade e colisão com plataformas
    player_update_position (player, level_4_info->num_platforms, level_4_info->platforms, LEVEL_4_WIDTH);
    
    update_player_sprite(player);
    update_player_bullets(level_4_info->camera_x,  level_4_info->player->buster, X_SCREEN, LEVEL_4_WIDTH);

    player_update_state(player, Y_SCREEN); //verifica se o estado do jogador é de tomando dano ou morto
    update_camera(player, &level_4_info->camera_x, X_SCREEN, LEVEL_4_WIDTH);

    if (player->is_dead) level_4_info->exit_level_4 = 1; 
    if (player->x >= LEVEL_4_WIDTH - player->width) {
        level_4_info->boss_defeated = 1; //como a fase não tem boss de verdade, andar até o final dela é acabar a fase
        level_4_info->exit_level_4 = 1;
    }
}
...
void show_level_4(ALLEGRO_EVENT *event, game_state *state, ALLEGRO_FONT *font, ALLEGRO_DISPLAY *disp,entities_sprites *sprites, level_4 *level_4_info, int X_SCREEN, int Y_SCREEN) {
    ...
    draw_default_background(level_4_info->camera_x, sprites->level_4_background, X_SCREEN, Y_SCREEN, level_4_info->FLOOR);

    for (int i=0; i<level_4_info->num_platforms; i++)
        draw_platform(level_4_info->camera_x, level_4_info->platforms[i], sprites->level_4_ground, X_SCREEN, Y_SCREEN);

    // Desenha o jogador por cima do cenário
    draw_player(font,state, level_4_info->camera_x, level_4_info->player, sprites, X_SCREEN, Y_SCREEN);
    draw_bullets(level_4_info->camera_x, level_4_info->player, sprites);
    
    al_flip_display();

    //descobrir se ele vai renascer ou não
    if (level_4_info->exit_level_4) {
        if (level_4_info->player->is_dead) {
            state->player_progress->Lifes -= 1;
            exit_level_4(state,level_4_info, sprites);

            if (state->player_progress->Lifes == 0) state->game_over = 1; //deu game over
            else state->level_4 = 1; //vai renascer no checkpoint
        } 
        else if (level_4_info->boss_defeated) {
            state->checkpoint = 0;
            state->player_progress->Level_4_completed = 1;
...
void create_level_4_platforms(level_4 *level_4_info, int X_SCREEN, int Y_SCREEN) {
    ...
    float default_ground_Y = Y_SCREEN - level_4_info->FLOOR;
    float default_ground_height = level_4_info->FLOOR;
    ...
    for (int i=0; i < level_4_info->num_platforms; i++) 
        level_4_info->platforms[i] = create_platform(pos_x[i],pos_y[i],width[i],height[i]);

}


void exit_level_4(game_state *state, level_4 *level_4_info, entities_sprites *sprites) {
    if (!level_4_info || !sprites) {
        return;
    }
 
    al_destroy_bitmap(sprites->level_4_background);
    al_destroy_bitmap(sprites->level_4_ground);
    ...
    player_destroy(level_4_info->player);

    for (int i = 0; i < level_4_info->num_platforms; i++) {
        if (level_4_info->platforms[i] != NULL) {
            platform_destroy(level_4_info->platforms[i]);
        }
    }
