...
level_2 *level_2_info_create(game_state *state, int X_SCREEN, int Y_SCREEN, int checkpoint) {
    level_2 *level_2_info = malloc(sizeof(level_2));
    if (!level_2_info) return NULL;
    level_2_info->camera_x = 0;
    level_2_info->FLOOR = Y_SCREEN*0.07;

    level_2_info->checkpoint = state->checkpoint;
    if (level_2_info->checkpoint == 0)
        level_2_info->player = create_player(state->player_progress->dificulty, 0, 0, X_SCREEN, Y_SCREEN);
 

    level_2_info->num_platforms = 2;
    create_level_2_platforms(level_2_info, X_SCREEN, Y_SCREEN);

    level_2_info->exit_level_2 = 0;
...
void start_level_2(game_state *state, level_2 **level_2_info, entities_sprites *sprites, int X_SCREEN, int Y_SCREEN, int checkpoint) {
    ...
    *level_2_info = level_2_info_create(state, X_SCREEN, Y_SCREEN, checkpoint);

    sprites->level_2_background = al_load_bitmap("images/cenario/level_2_background.png");
    sprites->level_2_ground = al_load_bitmap("images/cenario/level_2_ground.png");
    ...
    if (// Cenário
        !sprites->level_2_background || !sprites->level_2_ground || 
        // Jogador
    ...
    state->level_2_started = 1;
}

void level_2_update(level_2 *level_2_info, game_state *state, int X_SCREEN, int Y_SCREEN) {
    Player *player = level_2_info->player;

    //verifica gravidade e colisão com plataformas
    player_update_position (player, level_2_info->num_platforms, level_2_info->platforms, LEVEL_2_WIDTH);
    
    update_player_sprite(player);
    update_player_bullets(level_2_info->camera_x,  level_2_info->player->buster, X_SCREEN, LEVEL_2_WIDTH);

    player_update_state(player, Y_SCREEN); //verifica se o estado do jogador é de tomando dano ou morto
    update_camera(player, &level_2_info->camera_x, X_SCREEN, LEVEL_2_WIDTH);

    if (player->is_dead) level_2_info->exit_level_2 = 1; 
    if (player->x >= LEVEL_2_WIDTH - player->width) {
        level_2_info->boss_defeated = 1; //como a fase não tem boss de verdade, andar até o final dela é acabar a fase
        level_2_info->exit_level_2 = 1;
    }
}
...
void show_level_2(ALLEGRO_EVENT *event, game_state *state, ALLEGRO_FONT *font, ALLEGRO_DISPLAY *disp,entities_sprites *sprites, level_2 *level_2_info, int X_SCREEN, int Y_SCREEN) {
    ...
    draw_default_background(level_2_info->camera_x, sprites->level_2_background, X_SCREEN, Y_SCREEN, level_2_info->FLOOR);

    for (int i=0; i<level_2_info->num_platforms; i++)
        draw_platform(level_2_info->camera_x, level_2_info->platforms[i], sprites->level_2_ground, X_SCREEN, Y_SCREEN);

    // Desenha o jogador por cima do cenário
    draw_player(font,state, level_2_info->camera_x, level_2_info->player, sprites, X_SCREEN, Y_SCREEN);
    draw_bullets(level_2_info->camera_x, level_2_info->player, sprites);
    
    al_flip_display();

    //descobrir se ele vai renascer ou não
    if (level_2_info->exit_level_2) {
        if (level_2_info->player->is_dead) {
            state->player_progress->Lifes -= 1;
            exit_level_2(state,level_2_info, sprites);

            if (state->player_progress->Lifes == 0) state->game_over = 1; //deu game over
            else state->level_2 = 1; //vai renascer no checkpoint
        } 
        else if (level_2_info->boss_defeated) {
            state->checkpoint = 0;
            state->player_progress->Level_2_completed = 1;
...
void create_level_2_platforms(level_2 *level_2_info, int X_SCREEN, int Y_SCREEN) {
    ...
    float default_ground_Y = Y_SCREEN - level_2_info->FLOOR;
    float default_ground_height = level_2_info->FLOOR;
    ...
    for (int i=0; i < level_2_info->num_platforms; i++) 
        level_2_info->platforms[i] = create_platform(pos_x[i],pos_y[i],width[i],height[i]);

}


void exit_level_2(game_state *state, level_2 *level_2_info, entities_sprites *sprites) {
    if (!level_2_info || !sprites) {
        return;
    }
 
    al_destroy_bitmap(sprites->level_2_background);
    al_destroy_bitmap(sprites->level_2_ground);
    ...
    player_destroy(level_2_info->player);

    for (int i = 0; i < level_2_info->num_platforms; i++) {
        if (level_2_info->platforms[i] != NULL) {
            platform_destroy(level_2_info->platforms[i]);
        }
    }
