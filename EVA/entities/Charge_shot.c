...
bullet_2* bullet_2_create(float dificulty, unsigned short x, unsigned short y, int trajectory, bullet_2 *next) {
	...
	new_bullet->hit_boy_y = BULLET_2_WIDTH;																														
	new_bullet->trajectory = trajectory;																											
	new_bullet->next = next; 	
	new_bullet->frame_timer = 0;
	new_bullet->current_frame = 0;

	new_bullet->damage = 20/dificulty;
	return new_bullet;
}
...
bullet_3* bullet_3_create(float dificulty, unsigned short x, unsigned short y, int trajectory, bullet_3 *next) {
	...
	new_bullet->hit_boy_y = BULLET_3_WIDTH;																														
	new_bullet->trajectory = trajectory;																											
	new_bullet->next = next; 	
	new_bullet->frame_timer = 0;
	new_bullet->current_frame = 0;

	new_bullet->damage = 50/dificulty;
	return new_bullet;
}
